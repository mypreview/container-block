	...
	const container = {
		...
		onJump: () => {
			container.els.scrl.on( 'click', function( event ) {
				...
				const offset = $( container.vars.wpAdminBar ).length
						? $( container.vars.wpAdminBar ).height()
						: 0,
					target = $( this )
						.attr( 'href' )
						.replace( '#!', '#' )
		...
		svgShim: () => {
			/MSIE \d|Trident.*rv:/.test( navigator.userAgent ) &&
				[].forEach.call( container.els.svgs, function( target ) {
					! ( function( target ) {
						const color = window
							.getComputedStyle( target, null )
							.getPropertyValue( 'color' );
						let element = new XMLSerializer().serializeToString(
							target
						);
						element =
							'data:image/svg+xml,' +
							( element = ( element = ( element = ( element = ( element = ( element = element.replace(
								/currentColor/g,
								color
							) ).replace( /\s\s+/g, ' ' ) ).replace(
								/</g,
								'%3C'
							) ).replace( />/g, '%3E' ) ).replace(
								/#/g,
								'%23'
							) ).replace( /"/g, "'" ) );
